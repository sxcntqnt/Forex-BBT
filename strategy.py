...
class StrategyManager:
    def __init__(self, data_manager: DataManager, api, logger):
        ...
        self._price_groups = (
            data_manager._symbol_groups
        )  # Assuming this is a property in DataManager
    ...
    async def should_enter_trade(self, symbol: str) -> bool:
        ...
        return any(
            [
                await strategy.should_enter_trade(symbol, self.data_manager)
    ...
    def set_indicator_signal_compare(
        ...
        # Define the key.
        key = "{ind_1}_comp_{ind_2}".format(ind_1=indicator_1, ind_2=indicator_2)
    ...
    def rsi(
        self, period: int, method: str = "wilders", column_name: str = "rsi"
    ) -> pd.DataFrame:
    ...
    def rate_of_change(
        self, period: int = 1, column_name: str = "rate_of_change"
    ) -> pd.DataFrame:
    ...
    def bollinger_bands(
        self, period: int = 20, column_name: str = "bollinger_bands"
    ) -> pd.DataFrame:
        ...
        self._frame["band_upper"] = 4 * (
            self._frame["moving_std"] / self._frame["moving_avg"]
        )

        # Define the lower band
        self._frame["band_lower"] = (
            self._frame["close"] - self._frame["moving_avg"]
        ) + (2 * self._frame["moving_std"]) / (4 * self._frame["moving_std"])

        # Clean up before sending back.
        self._frame.drop(labels=["moving_avg", "moving_std"], axis=1, inplace=True)

        return self._frame

    def average_true_range(
        self, period: int = 14, column_name: str = "average_true_range"
    ) -> pd.DataFrame:
        ...
        self._frame["true_range_1"] = abs(
            self._frame["high"] - self._frame["close"].shift()
        )
        self._frame["true_range_2"] = abs(
            self._frame["low"] - self._frame["close"].shift()
        )

        # Grab the Max.
        self._frame["true_range"] = self._frame[
            ["true_range_0", "true_range_1", "true_range_2"]
        ].max(axis=1)
    ...
    def stochastic_oscillator(
        self, column_name: str = "stochastic_oscillator"
    ) -> pd.DataFrame:
        ...
        self._frame["stochastic_oscillator"] = (
            self._frame["close"]
            - self._frame["low"] / self._frame["high"]
            - self._frame["low"]
        )
    ...
    def macd(
        self, fast_period: int = 12, slow_period: int = 26, column_name: str = "macd"
    ) -> pd.DataFrame:
    ...
    def mass_index(
        self, period: int = 9, column_name: str = "mass_index"
    ) -> pd.DataFrame:
        ...
        # Grab the raw index.
        self._frame["mass_index_raw"] = (
            self._frame["mass_index_1"] / self._frame["mass_index_2"]
        )
    ...
    def force_index(
        self, period: int, column_name: str = "force_index"
    ) -> pd.DataFrame:
        ...
        self._frame[column_name] = self._frame["close"].diff(period) * self._frame[
            "volume"
        ].diff(period)
    ...
    def ease_of_movement(
        self, period: int, column_name: str = "ease_of_movement"
    ) -> pd.DataFrame:
        ...
        diff_divi_vol = (self._frame["high"] - self._frame["low"]) / (
            2 * self._frame["volume"]
        )
        ...
        # Clean up before sending back.
        self._frame.drop(labels=["ease_of_movement_raw"], axis=1, inplace=True)

        return self._frame

    def commodity_channel_index(
        self, period: int, column_name: str = "commodity_channel_index"
    ) -> pd.DataFrame:
        ...
        self._frame[column_name] = (
            self._frame["typical_price_mean"] / self._frame["typical_price_std"]
        )
    ...
    def standard_deviation(
        self, period: int, column_name: str = "standard_deviation"
    ) -> pd.DataFrame:
    ...
    def chaikin_oscillator(
        self, period: int, column_name: str = "chaikin_oscillator"
    ) -> pd.DataFrame:
        ...
        self._frame["money_flow_volume"] = (
            money_flow_multiplier_top / money_flow_multiplier_bot
        ) * self._frame["volume"]
        ...
        self._frame["money_flow_volume_10"] = self._frame[
            "money_flow_volume"
        ].transform(lambda x: x.ewm(span=10, min_periods=9).mean())

        # Calculate the Chaikin Oscillator.
        self._frame[column_name] = (
            self._frame["money_flow_volume_3"] - self._frame["money_flow_volume_10"]
        )
    ...
    def kst_oscillator(
        ...
        n3: int,
        n4: int,
        column_name: str = "kst_oscillator",
    ) -> pd.DataFrame:
        """Calculates the Mass Index indicator.
        ...
        # Calculate the ROC 1.
        self._frame["roc_1"] = self._frame["close"].diff(r1 - 1) / self._frame[
            "close"
        ].shift(r1 - 1)

        # Calculate the ROC 2.
        self._frame["roc_2"] = self._frame["close"].diff(r2 - 1) / self._frame[
            "close"
        ].shift(r2 - 1)

        # Calculate the ROC 3.
        self._frame["roc_3"] = self._frame["close"].diff(r3 - 1) / self._frame[
            "close"
        ].shift(r3 - 1)

        # Calculate the ROC 4.
        self._frame["roc_4"] = self._frame["close"].diff(r4 - 1) / self._frame[
            "close"
        ].shift(r4 - 1)
        ...
        self._frame[column_name] = 100 * (
            self._frame["roc_1_n"]
            + 2 * self._frame["roc_2_n"]
            + 3 * self._frame["roc_3_n"]
            + 4 * self._frame["roc_4_n"]
        )
